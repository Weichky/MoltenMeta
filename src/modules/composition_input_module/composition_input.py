...
_ELEMENT_PATTERN = __import__("re").compile(r"[A-Z][a-z]?")
...
class CompositionTool:
    def parseAndValidate(
        ...
        while pos < len(remaining) and elem_idx < len(element_symbols):
            ...
            if after_elem_pos >= len(remaining):
                ...
                elem_idx += 1
                continue
            num_match = __import__("re").match(r"(\d+\.?\d*)", remaining[after_elem_pos:])
            if num_match:
    ...
    def symbolToNumber(self, symbol: str) -> int | None:
        from modules.element_map.element_map import elemSymbolToId
        return elemSymbolToId(symbol)
